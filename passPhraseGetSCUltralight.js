...
if(!myArgs[0]) {
  console.log("USAGE: node " + thisFrilenamwe + " <password for keystore>")
  process.exit(1)
}
      ...
      if ((changes & this.SCARD_STATE_EMPTY) && (status.state & this.SCARD_STATE_EMPTY)) {
        ...
        reader.connect({ share_mode : this.SCARD_SHARE_SHARED }, function(err, protocol) {
          if (err) {
            ...
            reader.transmit(new Buffer.from([0xFF,0xB0,0x00,0x05,0x02]), 40, protocol, function(err, data) {
...
//process.exit()
              ...
              for(var i=0; i<bytes; i++) {
                ...
                if(byte === 3) {
//                  console.log(data,page,byte,"FF B0 00 " + thisHexPageNumber + " 04")
                  scCommands.push("FF B0 00 " + thisHexPageNumber + " 04")
                  page++
                  byte = 0
                } else 
                  byte++
